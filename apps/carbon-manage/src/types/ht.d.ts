...
declare namespace ht {
  ...
  namespace layout {
    export class AutoLayout {
      constructor(view: ht.graph.GraphView, config: any);
      setAnimate(boolean);
      layout(ward: string, func: Function);
    }
  }
   ...
   * @extends Node
   */
  export class Group extends ht.Node {
    /**
     * 判断Group对象是否处于展开状态
     * @return {boolean}
     */
  ...
  export class SelectionModel {
     ...
     * @return {List} 被选中的数据元素集合
     */
    toSelection(matchFunc: Function, scope?: any): List;
  ...
  namespace graph {
    ...
    export class GraphView {
       ...
       */
      deserialize(url: string, callback?: Function): void;
      /**
       ...
       * @param bound
       */
      layoutHTML(data: Data, html: DOM | Object, bound?: boolean): void;
  ...
  namespace Default {
     ...
     * @param actionType
     */
    function getConnectAction(gv: graph.GraphView, actionType?: string): any;
    ...
    function loadJS(urls: any[], callback: Function);
  }
  ...
  namespace Math {
    ...
    /**
     * 生成一个随机整数
     * @param min 当只有一个参数时，该参数为最大值，最小值按0计算
     * @param max 最大值
     */
    export function randomInt(min: number, max?: number): number;

    export function randomArray(arr: number[], len?: number): number[];
  }
