<template>
  <table-layout>
    <template #header>
      <div class="flex">
        <Breadcrumb v-model="queryParams.breadcrumbList" @changeLevel="changeLevel"></Breadcrumb>
    ...
    <el-table v-loading="tableLoading" :data="tableData" class="custom-table" height="100%">
      ...
      <el-table-column prop="name" label="名称" align="center">
        <template #default="{ row }">
          <el-button type="primary" link @click="addBreadcrumbList(row.name)">
            {{ row.name }}
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog append-to-body v-model="dialogVisible" top="30vh" draggable width="450px">
      <template #title>
        <div class="flex content-center">
          <el-text>
            {{ breadcrumbKeys[queryParams.level - 1] }}
          </el-text>
      ...
      >
        <el-form-item
          label="核算标准"
          v-if="!isEmpty(queryParams.breadcrumbList.get(breadcrumbKeys[0]))"
        >
          {{ queryParams.breadcrumbList.get(breadcrumbKeys[0]) }}
        </el-form-item>
        <el-form-item
          label="核算范围"
          v-if="!isEmpty(queryParams.breadcrumbList.get(breadcrumbKeys[1]))"
        >
          {{ queryParams.breadcrumbList.get(breadcrumbKeys[1]) }}
        </el-form-item>
        <el-form-item label="名称">
...
// TODO 根据breadcrumb刷新页面,新增和编辑操作
type BreadcrumbKeys = '核算标准' | '核算范围' | '核算项';
const breadcrumbKeys: BreadcrumbKeys[] = ['核算标准', '核算范围', '核算项'];
const dialogVisible = ref(false);
const dialogTitle = ref('');
const queryParams = ref<{
  name?: string;
  level: number;
  breadcrumbList: Map<BreadcrumbKeys, string>;
}>({
  level: 1,
  breadcrumbList: new Map([
    ['核算标准', ''],
    ['核算范围', ''],
    ['核算项', ''],
  ]),
...
const changeLevel = (level: number) => {
  queryParams.value.level = level;
  console.log(level);
};
const addBreadcrumbList = (label: string) => {
  if (queryParams.value.level != 3) {
    queryParams.value.breadcrumbList.set(breadcrumbKeys[queryParams.value.level - 1], label);
    // 进入下一级菜单
